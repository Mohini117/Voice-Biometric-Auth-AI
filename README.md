...
- Confirm the browser has microphone permissions
- Check that no other app is locking the microphone
- Use HTTPS or localhost during development

### Speech Recognition Returns Empty Text

- Test in Chrome or another Chromium-based browser
- Speak clearly and avoid noisy environments
- Confirm browser support for the Web Speech API

### Supabase Authentication Issues

- Verify `VITE_SUPABASE_URL` and `VITE_SUPABASE_PUBLISHABLE_KEY`
- Confirm your Supabase auth providers are enabled
- Check redirect URLs in the Supabase dashboard

### Voice Enrollment Does Not Persist

- Apply the migration in `supabase/migrations`
- Confirm the authenticated user can insert into `voice_profiles`
- Check the browser console and Supabase logs for insert or RLS errors

## Future Improvements

- liveness detection and replay-attack resistance
- model calibration and threshold tuning by environment
- stronger audit and observability tooling
- better cross-browser support
- admin dashboard for authentication analytics
- end-to-end and unit test coverage
- optional server-side biometric verification
