...
function computeDelta(frames: number[][]): number[][] {
  // If we have fewer than 3 frames, we can't compute proper deltas
  // Return frames as-is so deltaMean will be computed from available frames
  if (frames.length < 3) {
    return frames.length > 0 ? frames : [new Array(frames[0]?.length || 13).fill(0)];
  }
...
export function extractVoiceSignature(audioData: Float32Array): VoiceSignature {
  ...
  if (mfccFrames.length === 0) {
    throw new Error('Audio recording is too short to extract voice signature. Please record for at least 2.5 seconds.');
  }

  const mean = computeVoiceSignature(mfccFrames);
  
  if (mean.length === 0) {
    throw new Error('Failed to compute voice signature from audio');
  }
  ...
  if (!deltaMean || deltaMean.length === 0) {
    throw new Error('Failed to compute delta coefficients from voice signature');
  }
...
export function averageSignatures(signatures: VoiceSignature[]): VoiceSignature {
  ...
  // Validate all signatures have the same structure
  const firstSig = signatures[0];
  if (!firstSig.mean || firstSig.mean.length === 0) {
    throw new Error('Invalid signature: mean is empty');
  }

  const numCoeffs = firstSig.mean.length;
  
  // Check all signatures are valid
  for (const sig of signatures) {
    if (!sig.mean || sig.mean.length !== numCoeffs) {
      throw new Error('Inconsistent signature structure: mean length mismatch');
    }
    if (!sig.variance || sig.variance.length !== numCoeffs) {
      throw new Error('Inconsistent signature structure: variance length mismatch');
    }
    if (!sig.deltaMean || sig.deltaMean.length === 0) {
      throw new Error('Inconsistent signature structure: deltaMean is empty');
    }
  }
